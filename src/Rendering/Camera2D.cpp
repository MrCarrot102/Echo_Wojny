...
void Camera2D::recalculateViewMatrix()
{
  ...
  transform = glm::translate(transform,
                             glm::vec3(m_Width * 0.5f, m_Height * 0.5f, 0.0f));
  transform = glm::scale(transform, glm::vec3(m_Zoom, m_Zoom, 1.0f));
  transform = glm::translate(
      transform, glm::vec3(-m_Position.x - (m_Width * 0.5f),
                           -m_Position.y - (m_Height * 0.5f), 0.0f));

  m_ViewMatrix = transform;
  m_ProjectionViewMatrix = m_ProjectionMatrix * m_ViewMatrix;
}

glm::vec2 Camera2D::screenToWorld(const glm::vec2& screenCoords,
                                  const sf::Window& window)
{
  // kordy myszki
  // obracamy os y myszli bo 0, 0 to lewy gorny rog a w opeen gl to jest lewy
  // dolny roh
...
void Camera2D::stopPanning() { m_IsPanning = false; }
